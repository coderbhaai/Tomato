...
export default class ImageGallery extends React.Component {
  ...
  renderItem(item) {
    ...
    let alt = item.original.replace('.jpg', '').replace(/_/g, ' ').replace(/-/g, ' ').replace('/india/storage/app/public/destination/', '')
    return (
      <div>
        {
          item.imageSet ? (
            <picture onLoad={!this.isImageLoaded(item) ? onImageLoad : null} onError={handleImageError}>
              { item.imageSet.map(source => ( <source key={source.media} media={source.media} srcSet={source.srcSet} type={source.type} /> )) }
              <img className="image-gallery-image" alt={item.originalAlt} src={item.original} />
            </picture>
          ) : (
            <img 
  ...
  renderThumbInner(item) {
    ...
    let alt = item.thumbnail.replace('.jpg', '').replace(/_/g, ' ').replace(/-/g, ' ').replace('/india/storage/app/public/destination/', '')
    return (
